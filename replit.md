...
- **Presentation Studio** (`/presentations`): AI-powered slide creation module inspired by Skywork.ai. Wizard flow: Source Selection → Brief → AI Outline Review → AI Slide Generation → Editor. Sources: AI prompt, KPI data, Projects, Workflow, Reviews, Analytics. 6 themes (Executive Dark, Clean Light, Bold Purple, Fresh Teal, Minimal, Warm Amber). 8 slide types (title, agenda, content, two-column, data, quote, section, closing). Editor: left thumbnail panel, center canvas with inline edit, right AI refine panel + speaker notes + version history. Auto-saves every 30s. Version snapshots on manual save. DB tables: `presentations`, `presentation_versions`. AI routes: `/api/presentations/generate-outline`, `/api/presentations/:id/generate-slides`, `/api/presentations/:id/refine-slide`.
